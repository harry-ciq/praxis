...
func (r *UserRepo) CreateUser(ctx context.Context, username, email, name, avatarURL string) (*User, error) {
	...
	err := r.pool.QueryRow(ctx,
		`INSERT INTO users (username, email, name, avatar_url)
		 VALUES ($1, $2, $3, $4)
		 RETURNING id, username, email, name, bio, avatar_url, headline, created_at, updated_at`,
		username, email, name, avatarURL,
	).Scan(&u.ID, &u.Username, &u.Email, &u.Name, &u.Bio, &u.AvatarURL, &u.Headline, &u.CreatedAt, &u.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &u, nil
...
func (r *UserRepo) GetUserByID(ctx context.Context, id string) (*User, error) {
	var u User
	err := r.pool.QueryRow(ctx,
		`SELECT id, username, email, name, bio, avatar_url, headline, created_at, updated_at
...
func (r *UserRepo) UpdateUser(ctx context.Context, id string, name, bio, headline *string) (*User, error) {
	var u User
	err := r.pool.QueryRow(ctx,
		`UPDATE users
